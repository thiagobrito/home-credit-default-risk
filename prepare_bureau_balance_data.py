...
def prepare_bureau_balance():
    ...
    new_df = bureau_balance_df.pivot(index='SK_ID_BUREAU', columns='MONTHS_BALANCE',
                                     values='STATUS_NUMBER').reset_index()
    ...
    new_df = new_df.merge(bureau_balance_df.groupby(by='SK_ID_BUREAU')['RISK_SCORE'].sum().reset_index(),
    ...
    new_df.to_csv('dataset/processed_bureau_balance.csv', index=False)
    print('Done.')
